...
function createEditors(element = document.body) {
	...
	allEditors.forEach((editorEl) => {
		...
		if (
			editorEl.nextSibling.nodeType == Node.TEXT_NODE &&
			editorEl.nextSibling.textContent.trim() === ""
		) {
			editorEl.nextSibling.remove();
		}
		const upload_url = element
			.querySelector(`#${script_id}-ck-editor-5-upload-url`)
			.getAttribute("data-upload-url");
		const upload_file_types = JSON.parse(
			element
				.querySelector(`#${script_id}-ck-editor-5-upload-url`)
				.getAttribute("data-upload-file-types"),
		);
		const csrf_cookie_name = element
			.querySelector(`#${script_id}-ck-editor-5-upload-url`)
		...
		const config = JSON.parse(
			...
			(key, value) => {
				var match = value.toString().match(/^\/(.*?)\/([gimy]*)$/);
				if (match) {
		...
		ClassicEditor.create(editorEl, config)
			.then((editor) => {
				...
				if (editor.plugins.has("WordCount")) {
					...
					const wordCountWrapper = element.querySelector(
						`#${script_id}-word-count`,
					);
